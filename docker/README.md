# Open AD Kit: containerized workloads for Pilot.Auto
...
When using Open AD Kit as a development container, it is easy to use Pilot.Auto's all-in-one development container image, `ghcr.io/tier4/autoware:universe-devel-cuda`.
If you do not need the CUDA drivers, you can also use the smaller image `ghcr.io/tier4/autoware:universe-devel`.

```shell
$ git clone git@github.com:tier4/autoware.git
$ cd autoware
$ vcs import src < autoware.repos
$ docker run -it --rm \
  –v $PWD/src/universe/autoware_universe/XXX/autoware_YYY:/autoware/src/autoware_YYY \
  ghcr.io/tier4/autoware:universe-devel-cuda
$ colcon build --mixin debug compile-commands
$ source install/setup.bash
$ ros2 run --prefix "gdb -ex run --args" autoware_YYY ZZZ
```

For example, if you want to make modifications to [`autoware_universe/perception/autoware_bytetrack`](https://github.com/tier4/autoware_universe/tree/main/perception/autoware_bytetrack), you can execute the following commands to perform the volume mount and debug build and execution of only the `autoware_bytetrack`.
...
In the execution container, there is the all-in-one runtime container for Pilot.Auto, `ghcr.io/tier4/autoware:universe-cuda`, and the multi-containerized `ghcr.io/tier4/autoware:universe-***-cuda` for each component of Pilot.Auto.

The all-in-one execution container also has the autoware_launch package installed, allowing it to be started in the same way as a locally built Pilot.Auto.

```shell
git clone git@github.com:tier4/autoware.git
cd autoware
docker run -it --rm ghcr.io/tier4/autoware:universe-cuda
...
```shell
docker run -it --rm ghcr.io/tier4/autoware:universe-sensing-perception-cuda
...
This stage performs only the basic setup required for all Pilot.Auto images.
...
This stage is an Pilot.Auto Core runtime container. It only includes the dependencies given by `/rosdep-core-exec-depend-packages.txt` and the binaries built in the `core-devel` stage.
...
This stage is an Pilot.Auto Sensing/Perception runtime container. It only includes the dependencies given by `/rosdep-universe-sensing-perception-exec-depend-packages.txt` and the binaries built in the `universe-sensing-perception-devel` stage.
...
This stage is a Pilot.Auto Visualization runtime container. It only includes the dependencies given by `/rosdep-universe-visualization-exec-depend-packages.txt` and the binaries built in the `universe-visualization-devel` stage.
...
This stage is an Pilot.Auto Localization/Mapping runtime container. It only includes the dependencies given by `/rosdep-universe-localization-mapping-exec-depend-packages.txt` and the binaries built in the `universe-localization-mapping-devel` stage.
...
This stage is an Pilot.Auto Planning/Control runtime container. It only includes the dependencies given by `/rosdep-universe-planning-control-exec-depend-packages.txt` and the binaries built in the `universe-planning-control-devel` stage.
...
This stage is an Pilot.Auto Vehicle/System runtime container. It only includes the dependencies given by `/rosdep-universe-vehicle-system-exec-depend-packages.txt` and the binaries built in the `universe-vehicle-system-devel` stage.
...
This stage is a Pilot.Auto API runtime container. It only includes the dependencies given by `/rosdep-universe-api-exec-depend-packages.txt` and the binaries built in the `universe-api-devel` stage.
...
This stage provides an all-in-one development container to Pilot.Auto developers. By running the host's source code with volume mounting, it allows for easy building and debugging of Pilot.Auto.
...
This stage is an Pilot.Auto runtime container. It only includes the dependencies given by `/rosdep-exec-depend-packages.txt` and the binaries built in the `universe-devel` stage.
