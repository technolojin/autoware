...
build_images() {
    ...
    docker buildx bake --load --progress=plain -f "$SCRIPT_DIR/autoware-openadk/docker-bake.hcl" \
        ...
        --set "devel.tags=ghcr.io/tier4/autoware-openadk:latest-devel$image_name_suffix" \
        --set "prebuilt.tags=ghcr.io/tier4/autoware-openadk:latest-prebuilt$image_name_suffix" \
        --set "runtime.tags=ghcr.io/tier4/autoware-openadk:latest-runtime$image_name_suffix" \
        "${targets[@]}"
    set +x
}
